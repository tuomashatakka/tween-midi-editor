...
export const SHORTCUTS: Shortcut[] = [
  // Tools / draw mode
  { key: 'b', description: 'Toggle Draw mode', run: ({ dispatch, getState }) =>
  ...
  // Transport
  { key: ' ', description: 'Play / Stop', run: ({ dispatch, getState }) => {
  ...
  { key: '+', description: 'Zoom in', run: ({ dispatch, getState }) =>
      dispatch(zoomX({ factor: 1.2, anchorTicks: getState().viewport.scrollTicks })) },
  { key: '=', description: 'Zoom in', run: ({ dispatch, getState }) =>
      dispatch(zoomX({ factor: 1.2, anchorTicks: getState().viewport.scrollTicks })) },
  { key: '-', description: 'Zoom out', run: ({ dispatch, getState }) =>
